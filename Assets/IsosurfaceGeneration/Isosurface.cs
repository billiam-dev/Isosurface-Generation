    ...
    public class Isosurface : MonoBehaviour
    {
        ...
        public void Generate()
        {
            ...
            for (int i = 0; i < m_Chunks.Length; i++)
            {
                ...
                Chunk newChunk = Chunk.New(chunkIndex, m_ChunkSize);
        ...
        void UpdateChunk(int index)
        {
            ...
            int3 chunkOriginIndex = UnwrapChunkIndex(index) * m_ChunkSize;
        ...
        {
            // In a proper jobs implementation, chunks should store the desnity as a native array, generated by a density job.
            // However this is built for demo purposes, so the conversion is nessesary.
            NativeArray<float> density = new(densityMap.totalPoints, Allocator.Persistent);
            for (int i = 0; i < densityMap.totalPoints; i++)
                density[i] = densityMap.density[i];

            NativeList<Vertex> vertices = new(Allocator.Persistent);
            NativeList<ushort> indices = new(Allocator.Persistent);
            ...
            ProfileMarker.Begin();
            JobHandle jobHandle = mesherJob.Schedule(densityMap.totalPoints, default);
            jobHandle.Complete();
            ProfileMarker.End();
        ...
        {
            return (index.z * m_Dimentions.x * m_Dimentions.y) + (index.y * m_Dimentions.x) + index.x;
        }

        int3 UnwrapChunkIndex(int index)
        {
            int x = index % m_Dimentions.x;
            int y = index / m_Dimentions.x % m_Dimentions.y;
            int z = index / (m_Dimentions.x * m_Dimentions.y);
